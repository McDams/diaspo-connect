/**
 * Messagerie filleul <-> parrain. La messagerie est présentée comme
 * potentiellement modérée : bannière permanente + bouton de signalement sur
 * chaque message reçu, conformément aux règles de sécurité de la plateforme.
 */
(function () {
  ...
  async function init() {
    ...
    if (!user) return;
    await Layout.mountApp("filleul", "messagerie", user);
    currentUser = user;
