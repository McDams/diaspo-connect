/**
 * Recherche de mentors côté mentoré connecté : classement par compatibilité
...
(function () {
  ...
  async function requestMatching(mentorId) {
    ...
    await NotificationCenter.push(mentorUser.id, {
      ...
      text: `${DCUtils.escapeHtml(document.querySelector('.dc-current-user-name')?.textContent || 'Un mentoré')} souhaite être accompagné(e) par vous.`,
      link: "pages/mentor/demandes.html",
    });
  ...
  async function init() {
    ...
    if (!user) return;
    await Layout.mountApp("mentore", "recherche-mentors", user);
