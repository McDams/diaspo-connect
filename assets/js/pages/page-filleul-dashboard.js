(function () {
    ...
    if (!matching) {
      host.innerHTML = `
        <div class="dc-empty-state py-4">
          ...
          <h6>Vous n'avez pas encore de parrain/marraine</h6>
          <p class="small mb-3">Lancez une recherche pour recevoir des propositions compatibles avec votre profil.</p>
          <a href="recherche-parrains.html" class="btn btn-primary btn-sm">Rechercher un parrain</a>
        </div>`;
      return;
    }
  ...
  function renderOnboarding(mentee) {
    ...
    const steps = [
      ...
      { label: "Trouver un parrain / une marraine", done: hasAccompaniment },
  ...
  async function init() {
    ...
    if (!user) return;
    await Layout.mountApp("filleul", "dashboard", user);
