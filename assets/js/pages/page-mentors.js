/**
 * Page publique "Trouver un mentor" - liste filtrable des mentors/mentors
 * vérifiés, avec détail en modal. Les actions de matching redirigent les
 * visiteurs non connectés vers l'inscription mentoré.
 */
(function () {
  ...
  function render(results) {
    ...
    if (!results.length) {
      host.innerHTML = `<div class="col-12"><div class="dc-empty-state">
        ...
        <h6>Aucun mentor ne correspond à ces critères</h6>
  ...
  function openDetail(id) {
    ...
    DCUtils.qs("#detailModalBody").innerHTML = `
      ...
      <div class="row g-3 small">
        ...
        <div class="col-12"><strong>Mentorés actifs</strong><div class="text-muted-dc">${activeCount} / ${m.maxMentees}</div></div>
      </div>
      <div class="dc-banner dc-banner-info mt-3"><i class="bi bi-info-circle"></i><span>Pour envoyer une demande de matching, vous devez d'abord créer votre profil mentoré.</span></div>
    `;
  ...
  function requestFromVisitor(id) {
    DCUtils.toast("Créez votre profil mentoré pour envoyer une demande de matching.", "info");
    setTimeout(() => { window.location.href = "register-mentore.html"; }, 900);
  }
