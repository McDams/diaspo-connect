/**
 * Traitement des demandes de matching reçues par un mentor :
...
(function () {
  ...
  async function accept(matchingId) {
    if (!MatchingEngine.isMentorEligible(mentor, matchings)) {
      DCUtils.toast("Vous avez atteint votre quota de 2 mentorés actifs, impossible d'accepter cette demande.", "danger");
      return;
    }
  ...
  async function init() {
    ...
    if (!user) return;
    currentUser = user;
    await Layout.mountApp("mentor", "demandes", user);
