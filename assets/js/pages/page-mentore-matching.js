(function () {
  ...
  async function init() {
    ...
    if (!user) return;
    await Layout.mountApp("mentore", "matching", user);
    ...
    if (!matching) {
      document.getElementById("matching-body").innerHTML = `
        <div class="dc-empty-state">
          ...
          <p class="small">Lancez une recherche pour recevoir des propositions de mentorat compatibles.</p>
          <a href="recherche-mentors.html" class="btn btn-primary">Rechercher un mentor</a>
        </div>`;
      return;
    }
        ...
        <div class="col-lg-5">
          <div class="dc-card p-3">
            ...
            <div class="d-grid gap-2">
              ...
              <button class="btn btn-outline-danger btn-sm" id="report-btn"><i class="bi bi-flag me-1"></i>Signaler ce mentor</button>
    ...
    if (endBtn) {
      endBtn.addEventListener("click", () => {
        ConfirmModal.open({
          ...
          body: "Cette action mettra fin définitivement à votre accompagnement actuel. Vous pourrez ensuite rechercher un nouveau mentor. Confirmez-vous ?",
          confirmLabel: "Mettre fin",
          variant: "danger",
          onConfirm: async () => {
            const now = new Date().toISOString();
            matching.status = "terminee";
            matching.endReason = "Fin anticipée à la demande du mentoré";
            matching.statusHistory.push({ status: "terminee", date: now, note: "Accompagnement terminé à la demande du mentoré" });
