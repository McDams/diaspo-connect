(function () {
  ...
  function bindActions(container) {
    container.addEventListener("click", (e) => {
      ...
      ConfirmModal.open({
        ...
        body: "Le mentoré sera notifié et pourra rechercher un nouveau mentor. Confirmez-vous ?",
        confirmLabel: "Mettre fin",
        onConfirm: async () => {
          const matching = matchings.find((m) => m.id === btn.dataset.id);
          const now = new Date().toISOString();
          matching.status = "terminee";
          matching.endReason = "Fin décidée par le mentor";
          matching.statusHistory.push({ status: "terminee", date: now, note: "Accompagnement terminé par le mentor" });
          const mentee = mentees.find((m) => m.id === matching.menteeId);
  ...
  async function init() {
    ...
    if (!user) return;
    await Layout.mountApp("mentor", "mentores", user);
